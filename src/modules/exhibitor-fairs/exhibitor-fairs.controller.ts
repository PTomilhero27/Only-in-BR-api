...
export class ExhibitorFairsController {
  ...
  linkStall(
    ...
    return this.service.linkStallToFairByMe(
      user.id,
      params.fairId,
      params.stallId,
      query.purchaseId,
  ...
  @ApiOperation({
    summary:
      'Desvincular uma barraca minha de uma feira (devolve consumo da compra)',
  })
  unlinkStall(
    @CurrentUser() user: JwtPayload,
    @Param() params: UnlinkStallParamsDto,
  ): Promise<UnlinkStallResponseDto> {
    return this.service.unlinkStallFromFairByMe(
      user.id,
      params.fairId,
      params.stallId,
    );
