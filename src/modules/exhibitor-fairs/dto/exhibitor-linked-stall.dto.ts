...
export class ExhibitorLinkedStallDto {
  @ApiProperty({ example: 'ckv7w3z5p0001q8l1p2abcxyz', description: 'ID da barraca (Stall).' })
  @IsString()
  stallId!: string
  ...
  @ApiProperty({ enum: StallSize, example: StallSize.SIZE_3X3, description: 'Tamanho da barraca.' })
  @IsEnum(StallSize)
  stallSize!: StallSize

  @ApiProperty({ example: '2026-02-03T12:34:56.000Z', description: 'Quando foi vinculada (ISO).' })
  @IsString()
  linkedAt!: string
  ...
  // Resumo financeiro útil na lista de barracas vinculadas
  @ApiPropertyOptional({ example: 150000, description: 'Preço unitário da compra (centavos).' })
  @IsOptional()
  @IsInt()
  @Min(0)
  purchaseUnitPriceCents!: number | null

  @ApiPropertyOptional({ example: 150000, description: 'Total da compra (centavos).' })
  @IsOptional()
  @IsInt()
  @Min(0)
  purchaseTotalCents!: number | null

  @ApiPropertyOptional({ example: 50000, description: 'Entrada paga na compra (centavos).' })
  @IsOptional()
  @IsInt()
  @Min(0)
  purchasePaidCents!: number | null

  @ApiPropertyOptional({ example: 2, description: 'Quantidade de parcelas da compra.' })
  @IsOptional()
  @IsInt()
  @Min(0)
