...
export class InterestFairsController {
  ...
  @ApiResponse({
    status: 409,
    description: 'O interessado já está vinculado a esta feira.',
  })
  link(
  ...
  @ApiResponse({
    status: 200,
    description: 'Compras substituídas com sucesso.',
  })
  @ApiResponse({
    status: 400,
    description: 'Payload inválido / regras de negócio.',
  })
  @ApiResponse({
    status: 409,
    description: 'Edição bloqueada porque já houve consumo (usedQty > 0).',
  })
  patchPurchases(
    @Param('id') ownerId: string,
    @Param('fairId') fairId: string,
  ...
  @Patch(':fairId/stalls/:stallFairId/tax')
  @ApiOperation({
    summary: 'Definir taxa aplicada a uma barraca vinculada na feira',
    description:
      'Aplica uma FairTax (da própria feira) a um StallFair. ' +
      'A taxa é única por barraca e é gravada com snapshot (nome/percentual).',
  })
  @ApiParam({ name: 'id', description: 'ID do interessado (Owner)' })
  @ApiParam({ name: 'fairId', description: 'ID da feira vinculada' })
  @ApiParam({
    name: 'stallFairId',
    description: 'ID do StallFair (barraca vinculada)',
  })
  patchStallTax(
    @Param('id') ownerId: string,
    @Param('fairId') fairId: string,
    @Param('stallFairId') stallFairId: string,
    @Body() dto: UpdateStallFairTaxDto,
    @CurrentUser() user: JwtPayload,
  ) {
    return this.service.updateStallFairTax(
      ownerId,
      fairId,
      stallFairId,
      dto,
