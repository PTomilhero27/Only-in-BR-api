...
export class StallsService {
  ...
  async createByMe(userId: string, dto: UpsertStallDto) {
    ...
    if (duplicated)
      throw new ConflictException('Você já possui uma barraca com esse nome.');

    const stallType: StallType = dto.stallType;
    const stallSize: StallSize = this.resolveStallSize(
      stallType,
      dto.stallSize,
    );
  ...
  async updateByMe(userId: string, stallId: string, dto: UpsertStallDto) {
    ...
    if (duplicated)
      throw new ConflictException(
        'Você já possui outra barraca com esse nome.',
      );
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.stallMenuProduct.deleteMany({
        where: { category: { stallId } },
      });
